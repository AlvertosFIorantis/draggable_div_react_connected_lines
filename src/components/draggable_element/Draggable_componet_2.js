...
function Draggable_componet_2(props) {
  ...
  return (
    <div className="mydiv" ref={myComponent} id={props.id}>
      
      <button onClick={()=>props.StartConnection(props.id,myComponent.current)}>Click to start a connection</button>
      <button onClick={()=>props.EndConnection(props.id,myComponent.current)}>Click to end a connection</button>

    </div>
  )
}

export default Draggable_componet_2
